...
class inventory{
	...
	void showallProductinfo()
	{
		...
		{	tmp=head[i];
			...
			while(tmp!=NULL)
			{
			     ...
			     cout<<x<<") "<<tmp->item_id<<"    "<<tmp->p_name<<"           "<<tmp->rate<<"            "<<tmp->quantity<<endl;		   					tmp=tmp->next;
				flag=1;
			}
		}
	...
	void Get_Product_info(long long n)
	{
		...
		while(tmp!=NULL && tmp->item_id<=n)
		{
			...
			cout<<"                            ITEM LIST                     \n\n";
			cout<<"\nProduct ID       Quantity       Rate          Amount"<<endl;
			cout<<" "<<tmp->item_id<<"       "<<tmp->p_name<<"          "<<tmp->rate<<"            "<<tmp->quantity<<endl;		   							
				flag=1;
				break;
			}
...
int main()
{
	...
	item *pro;
	char re;
	long long Mid,i,loo=1,swi_ma,x=1,cusid,com=1,check,item_id,item_rate,item_qua;
